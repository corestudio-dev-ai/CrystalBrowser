...
<style>
  :root { --accent:#2F6BFF; --accent2:#8FB6FF; color-scheme:light; }
  * { box-sizing:border-box; margin:0; padding:0; }
  body { font-family:'Segoe UI',system-ui,sans-serif; color:#e9e9ff;
    ...
    min-height:100vh; padding:52px 20px; }
...
<div class="wrap">
  <div class="logo">
    <svg class="gem" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M5 3h14l3 6-10 12L2 9l3-6z" stroke="var(--accent)" stroke-width="1.5"
            stroke-linejoin="round" fill="rgba(47,107,255,.20)"/></svg>
    <div>
      <h1>Crystal Browser <span>2.0 ULTRA</span></h1>
      <div class="tag">A fast, light, private Chromium browser — redesigned.</div>
    </div>
  </div>
...
<script>
  var ACCENTS = ['#2F6BFF','#1E88E5','#00B4D8','#22C55E','#7C6CFF','#FF3D7E'];
  var FRAMES = ['#2F6BFF','#2B2F3A','#00B4D8','#1E88E5','#22C55E','#FF3D7E'];
  var THEMES = {{ThemesJs}};
  ...
  window.crystalSettings = function(s){
    markTheme(s.theme||'ultra');
    markEngine(s.searchEngine||'google');
    markAccent(s.accent);
    markFrame(s.frameColor||'#2F6BFF');
